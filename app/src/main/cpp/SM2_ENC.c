...
11.SM3_done
12.SM3_KDF
//compress the the message
//compress the rest message and output the hash value
//key deviding function base on SM3, generates key stream
Notes:
...
randK[SM2_NUMWORD]
pubKey
// a random number K lies in [1,n-1]
// public key of the cipher receiver
// original message
M[klen]
klen
// byte len of original message
Output:
Return:
...
SM2_Decrypt
SM2 decryption
SM2_KDF,Test_Point,SM3_init,SM3_process,SM3_done
Called By:
Input:
dB
// a big number lies in [1,n-2]
...
Output:
Return:
M[Clen-SM2_NUMWORD*3] // decrypted data
0: sucess
...
Calls:
SM2_ENC_SelfTest
test whether the SM2 calculation is correct by comparing the result with the
SM2_init,SM2_ENC,SM2_DEC
Called By:
Input:
NULL
Output:
...
///////////////////////////////////////////////////////////////////////////////////////////
int bytes_to_epoint(unsigned char *in,epoint *out)
{
	big x,y;
	x=mirvar(0);
	y=mirvar(0);
	bytes_to_big(SM2_NUMWORD,in,x);
	bytes_to_big(SM2_NUMWORD,in+SM2_NUMWORD,y);
	if(!epoint_set(x,y,0,out))
	{
		return 1;
	}
	return 0;
...
int SM2_KeyGeneration_w(unsigned char *ppriKey,unsigned char *ppubKey)
{
int i=0;
big x,y;
//y
...
SM2_Init_w();
priKey=mirvar(0);
bytes_to_big(SM2_NUMWORD,ppriKey,priKey);
pubKey=epoint_init();
...
epoint_get(pubKey,x,y);
//y
big_to_bytes(SM2_NUMWORD,x,ppubKey,1);
big_to_bytes(SM2_NUMWORD,y,ppubKey+SM2_NUMWORD,1);
//j
if(Test_PubKey(pubKey)!=0)
return 1;
else
...
int SM2_Encrypt_w(unsigned char* randK,unsigned char *ppubKey,unsigned char M[],int klen,unsigned char C[])
{
big C1x,C1y,x2,y2,rand;
epoint *C1,*kP,*S;
int i=0;
...
SM2_Init_w();
pubKey=epoint_init();
if(bytes_to_epoint(ppubKey,pubKey))
{
	return 11;
}
//j
...
int SM2_Decrypt_w(unsigned char *ddB,unsigned char C[],int Clen,unsigned char M[])
{
SM3_STATE md;
int i=0;
unsigned char x2y2[SM2_NUMWORD*2]={0};
...
SM2_Init_w();
dB=mirvar(0);
bytes_to_big(SM2_NUMWORD,ddB,dB);
//j
...
//if (tmp!=0)
//return tmp;
tmp=SM2_KeyGeneration_w(std_priKey,std_pubKey1);
if (tmp!=0)
return tmp;
if(memcmp(std_pubKey1,std_pubKey,SM2_NUMWORD*2)!=0)
return ERR_SELFTEST_KG;
//epoint_get(kG,x,y);
//big_to_bytes(SM2_NUMWORD,x,kGxy,1);
//big_to_bytes(SM2_NUMWORD,y,kGxy+SM2_NUMWORD,1);
...
//return ERR_SELFTEST_KG;
//encrypt data and compare the result with the standard data
tmp=SM2_Encrypt_w(std_rand,std_pubKey,std_Message,19,Cipher);
if(tmp!=0)
return tmp;
if(memcmp(Cipher,std_Cipher,19+SM2_NUMWORD*3)!=0)
return ERR_SELFTEST_ENC;
//decrypt cipher and compare the result with the standard data
tmp=SM2_Decrypt_w(std_priKey,Cipher,115,M);
if(tmp!=0)
return tmp;
if(memcmp(M,std_Message,19)!=0)
return ERR_SELFTEST_DEC;
